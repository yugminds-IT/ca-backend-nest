...
export class ClientFilesService {
  ...
  async uploadFilesForClient(
    ...
    if (!ORG_ROLES.includes(currentUser.role?.name as RoleName)) {
      throw new ForbiddenException('Only org users can upload files for clients');
    }
  ...
  async deleteFile(fileId: number, currentUser: User): Promise<{ message: string }> {
    const file = await this.repo.findOne({ where: { id: fileId } });
    if (!file) throw new NotFoundException('File not found');
    if (currentUser.role?.name === RoleName.CLIENT) {
      const myClientId = await this.getClientId(currentUser);
    ...
    try {
      await this.s3.delete(file.s3Key);
    } catch {
      // S3 deletion failure is non-fatal; DB record is still removed
    }
